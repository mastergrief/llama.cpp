...
void quantize_row_q1_75(const float * GGML_RESTRICT x, void * GGML_RESTRICT y, int64_t k) {
    quantize_row_q1_75_ref(x, y, k);
}
...
void ggml_vec_dot_q1_75_q8_0(int n, float * GGML_RESTRICT s, size_t bs, const void * GGML_RESTRICT vx, size_t bx, const void * GGML_RESTRICT vy, size_t by, int nrc) {
    const int qk = QK1_75;
    const int nb = n / qk;
    ...
    const block_q1_75 * GGML_RESTRICT x = vx;
    const block_q8_0  * GGML_RESTRICT y = vy;
    ...
    for (int i = 0; i < nb; i++) {
        ...
        // 4 chunks of 32 elements; chunk k covers digits [32k, 32k+32) of the
        // 128-digit block, stored as two 13-byte halves of 64 digits each.
        for (int k = 0; k < 4; k++) {
            const block_q8_0 * GGML_RESTRICT yb = &y[i * 4 + k];
            const float d1 = GGML_CPU_FP16_TO_FP32(yb->d);
            int sumi_block = 0;
            ...
            const int half = k / 2;
            const uint8_t * GGML_RESTRICT qs = &x[i].qs[13*half];
            const int8_t  * GGML_RESTRICT qy = yb->qs;

            const int base = (k % 2) * 32; // digit offset within the 64-digit half
            for (int j = 0; j < 32; ++j) {
                const int dj = base + j;
                static const uint8_t pow3[5] = {1, 3, 9, 27, 81};
                const int q = (qs[dj / 5] / pow3[dj % 5]) % 3;
                sumi_block += (q - 1) * qy[j];
            }
