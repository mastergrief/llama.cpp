...
// Q1_75: same ternary quantization as Q2_0 (codes clamped to {0,1,2}), stored
// base-3 packed: per block, two 13-byte halves of 64 digits each (5 per byte).
void quantize_row_q1_75_ref(const float * GGML_RESTRICT x, block_q1_75 * GGML_RESTRICT y, int64_t k) {
    static const int qk = QK1_75;
    ...
    for (int i = 0; i < nb; i++) {
        ...
        for (int j = 0; j < qk; ++j) {
            ...
            int q = (int)roundf(w * id) + 1;
            if (q < 0) q = 0;
            if (q > 2) q = 2; // ternary only: no +2 code in this format
            const int half      = j / 64;
            const int dj        = j % 64;
            const int byte_index = 13*half + dj / 5;
            const int digit_pos  = dj % 5;
            y[i].qs[byte_index] += (uint8_t)(q * pow3[digit_pos]);
        }
    }
}
...
void dequantize_row_q1_75(const block_q1_75 * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    static const int qk = QK1_75;
    ...
    for (int i = 0; i < nb; i++) {
        ...
        for (int j = 0; j < qk; ++j) {
            ...
            const int byte_index = 13*half + dj / 5;
            const int digit_pos  = dj % 5;
            const int q = (x[i].qs[byte_index] / pow3[digit_pos]) % 3;
            y[i*qk + j] = (q - 1) * d;
        }
    }
}
...
size_t quantize_q1_75(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    if (!quant_weights) {
        quantize_row_q1_75_ref(src, dst, (int64_t)nrow*n_per_row);
        return nrow * ggml_row_size(GGML_TYPE_Q1_75, n_per_row);
    }
    size_t row_size = ggml_row_size(GGML_TYPE_Q1_75, n_per_row);
    char * qrow = (char *)dst;
    for (int64_t row = 0; row < nrow; ++row) {
        quantize_row_q1_75_ref(src, (block_q1_75*)qrow, n_per_row);
        src += n_per_row;
        qrow += row_size;
    }
    return nrow * row_size;
...
bool ggml_validate_row_data(enum ggml_type type, const void * data, size_t nbytes) {
    ...
    switch (type) {
        ...
        case GGML_TYPE_Q1_75:
            {
                VALIDATE_ROW_DATA_D_F16_IMPL(block_q1_75, data, nb);
            } break;
        case GGML_TYPE_Q4_0:
            {
