...
#define VDR_Q1_75_Q8_1_MMVQ 1  // Process one 32-element chunk at a time for parallelism
...
static __device__ __forceinline__ float vec_dot_q1_75_q8_1(
    const void * __restrict__ vbq, const block_q8_1 * __restrict__ bq8_1, const int & kbx, const int & iqs) {

    const block_q1_75 * bq = (const block_q1_75 *) vbq + kbx;

    // Q1_75: 128 ternary codes {0,1,2} base-3 packed as two 13-byte halves of
    // 64 digits (5 per byte, last slot of each half unused). iqs selects the
    // 32-element chunk (0-3): half = iqs/2, and within the half the chunk
    // starts at digit 0 (even iqs) or digit 32 (odd iqs). Bytes are decoded
    // through a 243-entry LUT: 5 base-3 digits pre-spread into 2-bit fields.
    const float d = bq->d;

    const block_q8_1 * bq8_1_chunk = bq8_1 + iqs;

    const uint8_t * qs = bq->qs + 13 * (iqs >> 1);

    int L[7];
    const int b0 = (iqs & 1) ? 6 : 0;
#pragma unroll
    for (int k = 0; k < 7; ++k) {
        L[k] = kq1_75_lut[qs[b0 + k]];
    }

    int sumi = 0;
    if ((iqs & 1) == 0) {
        // code n = digit n: field n%5 of L[n/5]
#pragma unroll
        for (int j = 0; j < 8; ++j) {
            const int n0 = 4*j, n1 = 4*j + 1, n2 = 4*j + 2, n3 = 4*j + 3;
            const int vi = (((L[n0/5] >> (2*(n0%5))) & 3)      ) |
                           (((L[n1/5] >> (2*(n1%5))) & 3) <<  8) |
                           (((L[n2/5] >> (2*(n2%5))) & 3) << 16) |
                           (((L[n3/5] >> (2*(n3%5))) & 3) << 24);
        ...
        // code n = digit n+32 of the half = field (n+2)%5 of L[(n+2)/5]
#pragma unroll
        for (int j = 0; j < 8; ++j) {
            const int n0 = 4*j + 2, n1 = 4*j + 3, n2 = 4*j + 4, n3 = 4*j + 5;
            const int vi = (((L[n0/5] >> (2*(n0%5))) & 3)      ) |
                           (((L[n1/5] >> (2*(n1%5))) & 3) <<  8) |
                           (((L[n2/5] >> (2*(n2%5))) & 3) << 16) |
                           (((L[n3/5] >> (2*(n3%5))) & 3) << 24);
    ...
    // Same deferred-correction pattern as vec_dot_q2_0_q8_1 (symbol = code - 1).
    const float2 ds8f = __half22float2(bq8_1_chunk->ds);
    return d * (sumi * ds8f.x - ds8f.y);
}
