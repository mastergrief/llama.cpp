...
#ifdef GGML_FATTN_STUB_K_DOT
        // TIMING STUB: keep loads, drop dp4a+scale math (output invalid)
        sum += (float) (v ^ Q_q8[k_KQ_0/nthreads]) + Q_d;
#else
...
#ifdef GGML_FATTN_STUB_V_DEQ
        // TIMING STUB: keep loads, drop per-element convert+mul (output invalid)
        const float acc = (float) (int) (qs[0] | qs[ne-1]);
#pragma unroll
        for (int l = 0; l < ne; ++l) {
            ((float *) dst)[l] = d;
        }
