...
static __device__ __forceinline__ void dequantize_q1_75(const void * vx, const int64_t ib, const int iqs, float2 & v){
    const block_q1_75 * x = (const block_q1_75 *) vx;

    const float d = x[ib].d;

    // Base-3 packed ternary decoded through the shared 243-entry LUT
    // (5 digits pre-spread into 2-bit fields per byte).
    const int half_0  = iqs / 64;
    const int dj_0    = iqs % 64;
    const int half_1  = (iqs + 1) / 64;
    const int dj_1    = (iqs + 1) % 64;

    const int c0 = (kq1_75_lut[x[ib].qs[13*half_0 + dj_0/5]] >> (2*(dj_0 % 5))) & 3;
    const int c1 = (kq1_75_lut[x[ib].qs[13*half_1 + dj_1/5]] >> (2*(dj_1 % 5))) & 3;
