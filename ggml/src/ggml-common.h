...
// Q1_75: ternary {-1,0,+1} stored base-3, 5 values per byte, one FP16 scale per 128.
// Layout: two 13-byte halves per block; each half holds 64 digits (5 per byte,
// last digit slot of each half unused). 28 bytes / 128 weights = 1.75 bpw.
#define QK1_75 128
typedef struct {
    ggml_half d;             // delta (scale)
    uint8_t qs[26];          // base-3 packed ternary codes {0,1,2} -> symbol = code - 1
} block_q1_75;
static_assert(sizeof(block_q1_75) == sizeof(ggml_half) + 26, "wrong q1_75 block size/padding");
